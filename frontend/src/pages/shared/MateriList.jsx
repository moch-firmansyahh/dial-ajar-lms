...
import { deleteModul, deleteVideo } from "../../api/modul.api";
...
const MateriList = () => {
  ...
  const handleDelete = async (e, compositeId, category) => {
    e.stopPropagation();
    if (!window.confirm("Apakah Anda yakin ingin menghapus materi ini?"))
      return;

    const idVal = compositeId.split("-")[1];

    try {
      if (category === "video") {
        await deleteVideo(idVal);
      } else {
        await deleteModul(idVal);
      }
    ...
    <div>
      ...
      {isLoading ? (
        ...
        >
          {Array(4)
            ...
            .map((_, i) => (
              <Card
                key={`skel-materi-${i}`}
                className={`p-5 flex ${viewMode === "grid" ? "flex-col gap-5 h-[200px]" : "flex-row items-center gap-4"}`}
              >
          ...
          {filteredMateri.map((materi, idx) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`absolute ${isGrid ? "-right-4 -bottom-6" : "-right-6 -bottom-10"} ${style.watermarkColor} opacity-[0.03] group-hover:opacity-[0.07] group-hover:scale-110 group-hover:-rotate-6 transition-all duration-500 pointer-events-none`}
                >
                  {isVideo ? (
                    <MonitorPlay size={isGrid ? 120 : 150} />
                  ) : (
                    <BookOpen size={isGrid ? 120 : 150} />
                  )}
                </div>
                ...
                >
                  ...
                  <div className="flex-1 w-full pr-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[12px] font-medium text-slate-500">
                      <span
                        className={`${style.badge} border px-2 py-0.5 rounded-md font-medium uppercase text-[10px] tracking-wider`}
                      >
                      ...
                      {isVideo &&
                        materi.duration &&
                        materi.duration !== "Unknown" && (
                          <>
                            <span className="w-1 h-1 bg-slate-300 rounded-full" />
                            <span className="flex items-center gap-1">
                              <Clock size={12} /> {materi.duration}
                            </span>
                          </>
                        )}
                      <span className="w-1 h-1 bg-slate-300 rounded-full" />
                ...
                {/* Premium Button */}
                <div
                  className={`flex gap-2 w-full ${isGrid ? "mt-auto" : "shrink-0 sm:w-auto relative z-20"}`}
                  onClick={(e) => e.stopPropagation()}
                >
                  <button
                    onClick={() => handleDownload(materi.url)}
                    className={`flex items-center justify-center gap-2 transition-all duration-300 shadow-sm hover:scale-[1.02] active:scale-[0.98]
                      ${isGrid ? "flex-1 py-2.5 rounded-xl font-medium text-[13px]" : "w-11 h-11 rounded-full"}
                      ${
                        isVideo
                  ...
                  >
                    {isVideo ? (
                      <Play size={18} className={!isGrid ? "ml-1" : ""} />
                    ) : (
                      <ArrowDownToLine size={18} />
                    )}
                    {isGrid && (
                      <span>{isVideo ? "Tonton Video" : "Unduh File"}</span>
                    )}
                  </button>
                  ...
                  {isDosen && (
                    <button
                      onClick={(e) =>
                        handleDelete(e, materi.id, materi.category)
                      }
                      className={`flex items-center justify-center gap-2 transition-all duration-300 shadow-sm border border-red-200 bg-red-50 hover:bg-red-600 text-red-600 hover:text-white hover:border-red-600 hover:scale-[1.02] active:scale-[0.98]
                        ${isGrid ? "px-3 py-2.5 rounded-xl" : "w-11 h-11 rounded-full"}
                      `}
                      title="Hapus Materi"
                    >
                      <Trash2 size={18} />
        ...
        <EmptyState
          ...
          description="Materi pembelajaran belum ditambahkan untuk mata kuliah ini."
        />
      )}
    </div>
  );
