    ...
    <div className="max-w-[900px] mx-auto pb-24">
          ...
          {activeReply === 'main' && (
            <div className="mt-5 flex gap-3 items-center animate-slide-up-fade">
              <input 
                ...
                placeholder="Tulis balasan untuk topik ini..."
                className="flex-1 bg-white border border-primary/30 rounded-xl px-4 py-2.5 text-[14px] outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all shadow-sm"
              />
              <button 
                onClick={() => {
                  handleMainReply();
                ...
                className={`p-2.5 rounded-xl transition-all ${commentText.trim() ? 'bg-primary text-white hover:bg-primary-hover hover:scale-105 shadow-sm' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}
              >
                <Send size={18} />
              </button>
            </div>
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <h3 className="text-[15px] font-semibold text-slate-800">
          {replies.length + replies.reduce((acc, curr) => acc + (curr.replies?.length || 0), 0)} Balasan Diskusi
        </h3>
      </div>
