...
import { 
  ...
  PlusCircle, UploadCloud, FilePlus, HelpCircle, Award, MessageSquare 
} from 'lucide-react';
  ...
  return (
    <div className="max-w-[1200px] mx-auto pb-10 animate-slideUpFade">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 font-medium mt-1">Ringkasan aktivitas mengajar Anda hari ini</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        {isLoading 
          ...
          <Card key={idx} className="relative overflow-hidden flex items-center gap-4 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300 border border-slate-100 group">
            ...
            <div className={`relative z-10 w-14 h-14 ${stat.bg} ${stat.color} rounded-2xl flex items-center justify-center shadow-sm border border-white shrink-0`}>
              <stat.icon size={26} />
            </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {quickActions.map((action, idx) => (
            ...
            >
              <div className={`w-12 h-12 rounded-xl bg-white shadow-sm border border-slate-100 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300 ${action.color}`}>
                <action.icon size={24} />
              </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[17px] font-medium text-slate-800 tracking-tight">Mata Kuliah Saya</h2>
          <Button variant="ghost" size="sm" onClick={() => navigate('/matakuliah')} className="font-medium text-primary">Lihat Semua <ArrowRight size={16} className="ml-1" /></Button>
        </div>
