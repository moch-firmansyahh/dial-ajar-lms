...
async getMateri(filterMatkul, filterTipe) {
    const materiData = await this.repository.findAllByDosen(filterMatkul, filterTipe);
    ...
    return materiData.map(m => ({
        ...
        url: m.url || m.fileUrl,
        ukuran: m.ukuran,
        tanggal: m.tanggal.toISOString().slice(0, 10),
        diunduh: m.diunduh,
